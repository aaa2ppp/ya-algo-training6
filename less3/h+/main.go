...
type stackWithSum struct {
	stack stack[int]
}

func newStackWithSum() *stackWithSum {
	var s stackWithSum
	s.stack.push(0)
	return &s
}

func (s *stackWithSum) push(v int) {
	s.stack.push(s.stack.top() + v)
}

func (s *stackWithSum) pop() int {
	return s.stack.pop() - s.stack.top()
}

func (s *stackWithSum) sum(k int) int {
	return s.stack.top() - s.stack.get(k)
}
