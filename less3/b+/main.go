...
func solve(aa []int) []int {
	...
	stack.push(item{val: -1, idx: -1})

	for i := n - 1; i >= 0; i-- {
		v := aa[i]

		if debugEnable {
			log.Printf("%d: %v", v, stack)
		}
		...
		// это не влияет на общую асимптотику, но может ускорить усечение стека
		j := sort.Search(len(stack), func(i int) bool {
			return stack[i].val >= v
		})
		stack = stack[:j]

		res[i] = stack.top().idx
		stack.push(item{val: v, idx: i})
	}
...
func run(in io.Reader, out io.Writer) {
	...
	writeInts(bw, res, defaultWriteOpts())
}
