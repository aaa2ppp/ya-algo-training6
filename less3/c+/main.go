...
func (s *stack) push(v int) {
	...
	*s = append(*s, item{val: v, minimum: minimum})
}

func (s stack) minimum() int {
	n := len(s)
	return s[n-1].minimum
}
...
func newWindow(k int) *window {
	return &window{
		input:  make(stack, 0, k),
		output: make(stack, 0, k),
	}
}
...
func (w *window) pop() int {
	if w.output.empty() {
		for !w.input.empty() {
			w.output.push(w.input.pop())
		}
	}
	return w.output.pop()
}

func (w *window) minimum() int {
	if w.input.empty() {
		return w.output.minimum()
	}

	if w.output.empty() {
		return w.input.minimum()
	}

	return min(w.input.minimum(), w.output.minimum())
}
