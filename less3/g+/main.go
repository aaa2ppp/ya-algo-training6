...
func solve(b int, aa []int) int {
	n := len(aa)
	q := 0 // кол-во ожидающих в очереди
	t := 0 // суммарное время ожидания

	for i := 0; i < n; i++ {
		q += aa[i] // пополняем очередь
		t += q     // ждем минуту

		if debugEnable {
			log.Printf("aa[%d]=%d q=%d t=%d", i, aa[i], q, t)
		}

		q -= min(b, q) // обслуживаем b но не более чем в очереди
	}

	t += q // оставшиеся ждут еще минуту и расходятся

	if debugEnable {
		log.Printf("=: q=%d t=%d", q, t)
	}
