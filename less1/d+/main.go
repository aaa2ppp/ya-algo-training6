...
func task(i int, sc *bufio.Scanner, bw *bufio.Writer) {
	tRoom, tCond, err := scanTwoInt(sc)
	if err != nil {
		panic(fmt.Errorf("%d: %v", i, err))
	}
	mode, err := scanWord(sc)
	if err != nil {
		panic(fmt.Errorf("%d: %v", i, err))
	}
...
func run(in io.Reader, out io.Writer) {
	...
	for i := 1; i <= t; i++ {
		sc.Scan()
		line := sc.Bytes()
		tsc := bufio.NewScanner(bytes.NewReader(line))
