...
func solve2(aa, bb, pp []int) []int {
	...
	sort.Slice(oa, func(i, j int) bool {
		return oa[i].a > oa[j].a ||
			oa[i].a == oa[j].a && (oa[i].b > oa[j].b ||
				oa[i].b == oa[j].b && oa[i].id < oa[j].id)
	})

	sort.Slice(ob, func(i, j int) bool {
		return ob[i].b > ob[j].b ||
			ob[i].b == ob[j].b && (ob[i].a > ob[j].a ||
				ob[i].a == ob[j].a && ob[i].id < ob[j].id)
	})
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			res = append(res, oa[0].id)
			oa[0].done = true
		case 1:
			for ob[0].done {
				ob = ob[1:]
			}
			res = append(res, ob[0].id)
			ob[0].done = true
		}
	}
