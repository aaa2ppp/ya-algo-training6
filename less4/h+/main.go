...
func solve(graph [][]int32, aa []int32) (int, []int32) {
	n := len(graph)
	_ = n

	// чтобы каждая вершина была либо помечена, либо соединена ребром с помеченной
	if n == 2 {
		return int(aa[1]), []int32{1}
	}
	...
	var dfs func(node, prev int32)

	dfs = func(node, prev int32) {
		nodeC0, nodeC1 := 0, int(aa[node])
		...
		for _, neig := range graph[node] {
			...
			dfs(neig, node)
			c0, c1 := costs[neig].c0, costs[neig].c1
			nodeC0 += c1
			nodeC1 += min(c0, c1)
		}

		costs[node] = item{nodeC0, nodeC1}
	}

	nodes := make([]int32, 0, (n+1)/2) // max 1 realloc

	var dfs2 func(node, prev int32, prevMark bool)

	dfs2 = func(node, prev int32, prevMark bool) {
		mark := true
		if prevMark {
			mark = costs[node].c0 > costs[node].c1
		}
	...
	dfs(1, -1)
	cost := min(costs[1].c0, costs[1].c1)
...
func run(in io.Reader, out io.Writer, solve func([][]int32, []int32) (int, []int32)) {
	...
	aa := make([]int32, n+1)
	if err := scanInts(sc, aa[1:]); err != nil {
		panic(err)
	}
	...
	cost, nodes := solve(graph, aa)

	writeInts(bw, []int{cost, len(nodes)}, defaultWriteOpts())
	writeInts(bw, nodes, defaultWriteOpts())
}
