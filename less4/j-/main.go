...
type (
	...
	Edge      [2]int
	Edges     []Edge
	SolveFunc func(Edges, Graph) int
)

func solve(edges Edges, graph Graph) (count int) {
	if debugEnable {
		log.Println("=> solve")
	}
	...
	dfs = func(node, prev int) (size, count int) {
		if debugEnable {
		...
		if len(graph[node].up) > 1 {

			// отстегиваем узел
			for i, neig := range graph[node].up {
				if neig == prev {
					n := len(graph[node].up)
					graph[node].up[i] = graph[node].up[n-1]
					graph[node].up = graph[node].up[:n-1]
					break
				}
			}

			// находим корень
			root := graph[node].up[0]
			for len(graph[root].up) != 0 {
				root = graph[root].up[0]
			}

			// вычисляем дерево
			size, count = dfs(root, 0)
			return size, count
		}
